...
export const PlaylistsCard = ({ title, description, imageUrl, id }) => {
  return (
      <Link
          to={`/playlist/${id}`}
      ...
      >
        <div className="mb-4 relative flex justify-center items-center">
          <img
              src={imageUrl}
              alt="Album"
              className="w-48 h-48 rounded-xl drop-shadow-2xl object-cover"
          />
        ...
        <div>
          <h5 className="font-medium text-gray-100 mb-2 text-wrap">{title}</h5>
          <p className="text-gray-400 text-sm break-all text-wrap">{description}</p>
        </div>
      </Link>
  );
};
